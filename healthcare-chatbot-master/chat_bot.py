...
def calc_condition(exp,days):
    sum=0
    for item in exp:
         sum=sum+severityDictionary[item] #severity based on the days and symptoms
...
def getfood_avoid():
    ...
    with open('healthcare-chatbot-master/MasterData/Disease_avoid_food.csv', mode='r', newline='') as csv_file:
        ...
        line_count = 0
        for row in csv_reader:
            try: _prec={row[0]:[row[1],row[2],row[3],row[4]]}
            except: 
                try: _prec={row[0]:[row[1],row[2],row[3]]}
                except: _prec={row[0]:[row[1],row[2]]}
            food_avoid.update(_prec)

def getInfo():
    print("----------------AIMED Disease Predictor----------------------")
    print("\nYour Name: ",end="")
    name=input("")
    print("Hello, ",name)
...
def tree_to_code(tree, feature_names):
    ...
    while True:
        disease_input = input("").replace(" ","_").lower()
        conf,cnf_dis=check_pattern(chk_dis,disease_input)
        if conf==1:
            print("searches related to input: ")
            for num,it in enumerate(cnf_dis):
                print(num,")",it.replace("_"," ").capitalize())
            if num!=0:
                conf_inp = int(input(f"Select the one you meant (option 0 - {num}): "))
            else:
                conf_inp=0
            ...
            print("Enter any other symptom: ")
    while(True):
        try: 
            num_days=int(input(f"Okay, from how many days are you having {disease_input}: "))
            break
        except: print("Enter valid value")
    ...
    def recurse(node, depth):
        ...
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            print(f"Avoid these food items for {present_disease[0]}:")
